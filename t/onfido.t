...
is_deeply( $src->as_arrayref->get->[0],
    $app, 'the most recent applicants is the one we created just now' );
...
is_deeply( $src->as_arrayref->get->[0],
    $doc, 'the most recent doc is the one we created just now' );
...
isa_ok( $src, 'Ryu::Source', 'the applicant list is a Ryu::Source' );
is_deeply( $src->as_arrayref->get->[0],
    $photo, 'the most recent photo is the one we created just now' );
...
is $check->download_uri,
  $onfido->endpoint( 'check_download', check_id => $check->id ),
  'Expected download uri';
...
is_deeply( $src->as_arrayref->get->[0],
    $check, 'the most recent check is the one we created just now' );

# get report
my ( $report, $report2 );
lives_ok {
    $report =
      $check->reports->filter( name => 'document' )->as_arrayref->get->[0]
}
...
is_deeply(
    ...
    [
        ...
        {
            GET =>
              $onfido->endpoint( 'document_download', $download_document->%* )
        },
        {
...
is_deeply( $src->as_arrayref->get->[0]->id,
    $check->id, 'expected check returned' );
