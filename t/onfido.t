...
# applicant_update
my $applicant_update = {
    first_name => 'Jack', applicant_id => $app->id
};
lives_ok { $onfido->applicant_update($applicant_update->%*)->get; } "update applicant ok ";
...
lives_ok { $src = $onfido->document_list($document_list->%*) } "document list ok";
...
my $doc2;
my $document_details = {
    applicant_id => $app->id, document_id => $doc->id
};
lives_ok { $doc2 = $onfido->get_document_details($document_details->%*)->get } 'get doc ok';
...
lives_ok { $content2 = $onfido->download_document($download_document->%*)->get } 'download doc ok';
...
my $live_photo_upload = {
    applicant_id => $app->id, filename => 'photo1.jpg', data => 'photo ' x 50
};
lives_ok { $photo = $onfido->live_photo_upload($live_photo_upload->%*)->get } 'upload photo ok';
...
lives_ok { $photo2 = $onfido->get_photo_details($get_photo_details->%*)->get } 'get photo ok';
is_deeply($photo2, $photo, 'get result is right');

# download_photo
my $photo_download = {
    live_photo_id => $photo->id
};
lives_ok { $content = $onfido->download_photo($photo_download->%*)->get } 'download doc ok';
...
# sdk_token
my $sdk_token = {
    applicant_id => $app->id, referrer => 'https://*.example.com/example_page/*'
};
my $token;
...
lives_ok { $onfido->applicant_delete($applicant_delete->%*)->get } "delete ok";

is_deeply([@api_hits], [
    { POST => $onfido->endpoint('applicants'), body => $applicant_create},
    { PUT => $onfido->endpoint('applicant', $applicant_update->%*), body => $applicant_update},
    { GET => $onfido->endpoint('applicant', $applicant_get->%*)},
    { GET => $onfido->endpoint('applicants')},
    { GET => $onfido->endpoint('applicant', $applicant_get->%*)},
    { POST => $onfido->endpoint('documents'), body => $document_upload},
    ...
    { POST => $onfido->endpoint('photo_upload'), body => $live_photo_upload},
    ...
    { POST => $onfido->endpoint('sdk_token'), body => $sdk_token},
