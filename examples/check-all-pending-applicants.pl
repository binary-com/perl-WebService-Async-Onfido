...
$onfido->applicant_list
    ->map(async sub {
        try {
            ...
            $log->tracef('Have %d checks for applicant %s', 0 + @checks, $applicant->as_string);
            return 0 if @checks;
            my @docs = await $applicant->documents->as_list;
            my @photos = await $applicant->photos->as_list;
            $log->tracef('Have %d documents, %d photos for applicant %s', 0 + @docs, 0 + @photos, $applicant->as_string);
            ...
            $log->infof('Starting check for applicant %s', $applicant->as_string);
            return await $applicant->check(
                ...
                reports => [qw(document facial_similarity)],
