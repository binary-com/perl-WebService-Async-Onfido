package WebService::Async::Onfido::Base::Document;

use strict;
use warnings;

use utf8;

# VERSION

=head1 NAME

WebService::Async::Onfido::Base::Document - represents data for Onfido

=head1 DESCRIPTION

This is autogenerated from the documentation in L<https://documentation.onfido.com>.

=cut

sub new {
    my ($class, %args) = @_;
    Scalar::Util::weaken($args{onfido}) if $args{onfido};
    bless \%args, $class;
}

=head1 METHODS
=head2 id

The unique identifier of the document.

=cut

sub id : method { shift->{ id } }

=head2 created_at

The date and time at which the document was uploaded.

=cut

sub created_at : method { shift->{ created_at } }

=head2 href

The URI of this resource.

=cut

sub href : method { shift->{ href } }

=head2 download_href

The URI that can be used to download the document.

=cut

sub download_href : method { shift->{ download_href } }

=head2 file_name

The name of the uploaded file.

=cut

sub file_name : method { shift->{ file_name } }

=head2 file_type

The file type of the uploaded file.

=cut

sub file_type : method { shift->{ file_type } }

=head2 file_size

The size of the file in bytes.

=cut

sub file_size : method { shift->{ file_size } }

=head2 type

The type of document.  The possible values are detailed below..

=cut

sub type : method { shift->{ type } }

=head2 side

The side of the document, if applicable.  The possible values are front and back.

=cut

sub side : method { shift->{ side } }

=head2 issuing_country

The issuing country of the document, in 3-letter ISO code, specified when uploading it..

=cut

sub issuing_country : method { shift->{ issuing_country } }

1;

__END__

=head1 AUTHOR

binary.com C<< BINARY@cpan.org >>

=head1 LICENSE

Copyright binary.com 2019. Licensed under the same terms as Perl itself.

